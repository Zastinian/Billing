...
export const POST: APIRoute = async ({ cookies, request, redirect, rewrite, params }) => {
  ...
  if (c.success === true && c.clientId !== null) {
    ...
    if (!type) {
      return redirect("/admin/clients?type=danger&msg=admin.client.verify.error");
    }
    if (type !== "yes" && type !== "no") {
      return redirect("/admin/clients?type=danger&msg=admin.client.verify.error");
    }
    client.emailVerifiedAt = new Date();
    client.isVerified = type === "yes" ? 1 : 0;
    client.updatedAt = new Date();
    await clients.save(client);
    return redirect(`/admin/clients/${client.id}?type=success&msg=admin.client.verify.success`);
  }
  return redirect("/");
};
