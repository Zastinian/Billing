...
export const POST: APIRoute = async ({ cookies, request, redirect, rewrite, params }) => {
  ...
  if (c.success === true && c.clientId !== null) {
    ...
    if (!data.password || !data.password_confirmation) {
      return redirect(`/admin/clients/${client.id}?type=danger&msg=admin.client.password.error`);
    }
    if (data.password !== data.password_confirmation) {
      return redirect(`/admin/clients/${client.id}?type=danger&msg=admin.client.password.error`);
    }
    ...
    return redirect(`/admin/clients/${client.id}?type=success&msg=admin.client.password.success`);
  }
  return redirect("/");
};
